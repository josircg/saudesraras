...
class ResourceList(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            ...
            serializerReturn = ResourceSerializer(Resource.objects.get(pk=serializer.data.get('id')),
...
class TrainingResourceList(APIView):
    ...
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            serializer.save(request)
            serializerReturn = TrainingResourceSerializer(Resource.objects.get(pk=serializer.data.get('id')),
                                                          context={'request': request})
            return Response(serializerReturn.data, status=HTTP_201_CREATED)
        return Response(serializer.errors, status=HTTP_400_BAD_REQUEST)
