...
from projects.models import Project, Topic
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-n', '--new', type=str, help='Create new Map (requires Map Name)')
    ...
    def handle(self, *args, **options):
        if options['auth']:
            ...
            response = requests.post(f'{endpoint}/visao2/api2/categories', json=record)
            if response.status_code == 201:
            ...
            response = requests.post(f'{endpoint}/visao2/api2/grup-categories', json=record, headers=auth_header)
            if response.status_code == 201:
            ...
            cookies = visao.authenticate()
            endpoint = settings.VISAO_URL
            query = {'id.equals': options['info']}
            response = requests.get(f'{endpoint}/visao2/api2/grup-categories', params=query)
            if response.status_code == 200:
            ...
            response = requests.get(f'{endpoint}/visao2/api2/categories', params=query, headers=auth_header)
            if response.status_code == 200:
            ...
            query = {'ownerId.equals': record['id'], 'type': 'LAYER'}
            ...
            print(f'{endpoint}/visao2/viewGroupCategory?l={settings.VISAO_LAYER}'
